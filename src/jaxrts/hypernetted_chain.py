...
def pair_distribution_function_HNC(V_s, V_l, Ti, ni):
    ...
    def condition(val):
        g_r, g_r_old = val
        return (jnp.max(jnp.abs(g_r - g_r_old)) > delta)
            
    def step(val):
        g_r, _ = val
        
        Ns_r = jnp.zeros_like(g_r)
        ...
        Ns_r = jnp.fft.fft(Ns_k, norm="backward", axis=2)

        g_r_new = jnp.exp(Ns_r - (beta * V_s).m_as(ureg.dimensionless))

        return g_r_new, g_r
    
    g_r = jax.lax.while_loop(condition, step, (g_r, g_r_old))
