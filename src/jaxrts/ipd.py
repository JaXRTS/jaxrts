...
All these implementations and formulas are taken from 'Modifications of Bound Statesin Dense Plasma Environments' by Rory A. Baggott.
...
def ipd_stewart_pyatt(
    ...
    """
    The correction to the ionization potential in the Stewart-Pyatt model using the small bound state approximation.
    ...
    Note: The Stewart-Pyatt value is always below both the Debye and ion sphere results.
    """
    ...
    s = 1 / (kappa * R_0)
    ipd_shift = -(
        (3 * (Zi+1) * ureg.elementary_charge**2 / (2 * 4 * jnp.pi * 1 * ureg.epsilon_0 * R_0)) * ((1 + s ** 3) ** (2/3) - s**2)
    )
...
def ipd_ecker_kroell(
    ...
    This model is similar to the model of Stewart-Pyatt and divided the radial dimension into three regions.
    ...
    n_c = (3 / (4 * jnp.pi)) * (
        4 * jnp.pi * 1 * ureg.epsilon_0 * ureg.boltzmann_constant * Te / ureg.elementary_charge**2
    ) ** 3
...
def ipd_pauli_blocking(Zi: float, ne: Quantity, ni: Quantity, Te: Quantity, Ti: Quantity) -> Quantity:
    ...
    def integrand(p):
        ...
        res = p**2 / (1 + a_Z**2 * p**2) ** 3 * (
            1
            / (
                jnpu.exp((((1 * ureg.hbar * p)**2 / (2 * ureg.electron_mass) - chem_pot) / (1 * ureg.boltzmann_constant * Te)))
                + 1
            )
        )
