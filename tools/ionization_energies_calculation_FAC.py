...
# --- Configuration Data ---
NEUTRAL_CONFIGS = {
    1: ('H', '1s1'), 2: ('He', '1s2'), 3: ('Li', '1s2 2s1'), 4: ('Be', '1s2 2s2'),
    5: ('B', '1s2 2s2 2p1'), 6: ('C', '1s2 2s2 2p2'), 7: ('N', '1s2 2s2 2p3'),
    8: ('O', '1s2 2s2 2p4'), 9: ('F', '1s2 2s2 2p5'), 10: ('Ne', '1s2 2s2 2p6'),
    11: ('Na', '1s2 2s2 2p6 3s1'), 12: ('Mg', '1s2 2s2 2p6 3s2'),
    13: ('Al', '1s2 2s2 2p6 3s2 3p1'), 14: ('Si', '1s2 2s2 2p6 3s2 3p2'),
    15: ('P', '1s2 2s2 2p6 3s2 3p3'), 16: ('S', '1s2 2s2 2p6 3s2 3p4'),
    17: ('Cl', '1s2 2s2 2p6 3s2 3p5'), 18: ('Ar', '1s2 2s2 2p6 3s2 3p6'),
    19: ('K', '1s2 2s2 2p6 3s2 3p6 4s1'), 20: ('Ca', '1s2 2s2 2p6 3s2 3p6 4s2'),
    21: ('Sc', '1s2 2s2 2p6 3s2 3p6 3d1 4s2'), 22: ('Ti', '1s2 2s2 2p6 3s2 3p6 3d2 4s2'),
    23: ('V', '1s2 2s2 2p6 3s2 3p6 3d3 4s2'), 24: ('Cr', '1s2 2s2 2p6 3s2 3p6 3d5 4s1'),
    25: ('Mn', '1s2 2s2 2p6 3s2 3p6 3d5 4s2'), 26: ('Fe', '1s2 2s2 2p6 3s2 3p6 3d6 4s2'),
    27: ('Co', '1s2 2s2 2p6 3s2 3p6 3d7 4s2'), 28: ('Ni', '1s2 2s2 2p6 3s2 3p6 3d8 4s2'),
    29: ('Cu', '1s2 2s2 2p6 3s2 3p6 3d10 4s1'), 30: ('Zn', '1s2 2s2 2p6 3s2 3p6 3d10 4s2'),
    31: ('Ga', '1s2 2s2 2p6 3s2 3p6 3d10 4s2 4p1'), 32: ('Ge', '1s2 2s2 2p6 3s2 3p6 3d10 4s2 4p2'),
    33: ('As', '1s2 2s2 2p6 3s2 3p6 3d10 4s2 4p3'), 34: ('Se', '1s2 2s2 2p6 3s2 3p6 3d10 4s2 4p4'),
    35: ('Br', '1s2 2s2 2p6 3s2 3p6 3d10 4s2 4p5'), 36: ('Kr', '1s2 2s2 2p6 3s2 3p6 3d10 4s2 4p6')
}
...
def create_ion_config(neutral_config, charge):
    if charge == 0: return neutral_config
    if charge < 0: raise ValueError("Anions not supported.")
    parts = neutral_config.split()
    orbitals = [(*parse_orbital(p), p) for p in parts if parse_orbital(p)]
    for _ in range(charge):
        if not orbitals: break
        orbitals.sort(key=lambda x: (x[0], x[1]), reverse=True)
        n, l, occupancy, original_part = orbitals[0]
        if occupancy > 1:
            orbitals[0] = (n, l, occupancy - 1, f"{n}{list(ORBITAL_L_VALUES.keys())[l]}{occupancy - 1}")
        else:
            orbitals.pop(0)
    final_parts = [orb[3] for orb in sorted(orbitals, key=lambda x: (x[0], x[1]))]
    return " ".join(final_parts)

def get_occupied_orbitals(config):
    return [re.match(r"(\d+[spdfgh])", part).group(1) for part in config.split()]
...
def _run_single_pfac_config(element_z, config_string):
    ...
    try:
        ...
        raise ImportError("The 'pfac' library is required but not installed. Please run 'pip install pfac'.")
        ...
        with open(text_file, 'r') as f:
            for line in f:
                if line.strip().startswith('E0'):
                    ...
                    break
        if not found:
            raise ValueError(f"Could not find 'E0' energy in FAC output for config '{config_string}'.")
        return energy
    finally:
...
def create_final_config(initial_config, orbital_to_remove):
    ...
    if not found_orbital:
        raise ValueError(f"Orbital '{orbital_to_remove}' not found in config '{initial_config}'.")
...
def run_in_worker(func, *args):
    ...
    p = multiprocessing.Process(target=_worker_target, args=(queue, func, *args))
    p.start()
...
def calculate_ionization_energy(element_z, initial_config, orbital_to_remove):
    ...
    e_initial = run_in_worker(_run_single_pfac_config, element_z, initial_config)
    
    if not final_config.strip():
        e_final = 0.0
    else:
        e_final = run_in_worker(_run_single_pfac_config, element_z, final_config)
...
def main():
    ...
    print(f"Batch calculation started at: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    for z in element_z_range:
        ...
        for charge in range(z):
            try:
                ...
                if not ion_config:
                    print(f"  Charge = +{charge}: Skipped (no electrons left).")
                    continue
                
                print(f"  Charge = +{charge}, Config = '{ion_config}'")
                occupied_orbitals = get_occupied_orbitals(ion_config)
                
                ion_results = {'config': ion_config, 'ionization_energies_eV': {}}
                element_data['ions'][charge] = ion_results

            except Exception as e:
                element_data['ions'][charge] = {'config': 'ERROR', 'ionization_energies_eV': {'error': str(e)}}
                continue

            for orbital in occupied_orbitals:
                print(f"    -> Calculating removal from {orbital}...", end='', flush=True)
                try:
                    energy_val = calculate_ionization_energy(z, ion_config, orbital)
                    ...
                    print(f" Failed ({e})")
                    ion_results['ionization_energies_eV'][orbital] = error_message
    ...
    try:
        ...
        with open(py_filename, 'w', encoding='utf-8') as f:
            ...
            f.write("# Ionization Energy Data Generated by FAC-code: https://github.com/flexible-atomic-code/fac\n")
            f.write("# This data was generated by the script 'ionization_energies_calculation_FAC.py' script.\n\n")
            f.write("ENERGY_DATA = ")
