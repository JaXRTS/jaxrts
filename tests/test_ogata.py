...
def fourier_transform_hankel(rvals, fvals, N, h):
    ...
    f_interp = scipy.interpolate.interp1d(r, fvals * np.sqrt(rvals), bounds_error = False, fill_value = (np.nan, 0.0))
...
f = lambda r: 1 / r * (np.exp(-r))
# f_aux = lambda r: f(r)
# Sample Function

k, transform = fourier_transform_hankel(r, f(r), 10000, 0.001)              # Return the transform of f at k.
...
for i in range(10):
    ...
    # f_fft = partial(fourier_transform_ogata(k[:,jnp.newaxis], rvals = r, fvals = f(r), N =10000, h = 0.0001)
    f_fft = jax.vmap(partial(fourier_transform_ogata, rvals = r, fvals = f(r), N =250, h = 0.0001))(k)
    # f_fft = np.vectorize(partial(fourier_transform_ogata, rvals = r, fvals = f(r), N =10000, h = 0.0001))(k)
    print("t=", time.time()-t0, "s")
# f_fft_fft = np.vectorize(partial(fourier_transform_ogata, rvals = k, fvals = f_fft, N = h_opt[2], h = h_opt[0]))(r)
